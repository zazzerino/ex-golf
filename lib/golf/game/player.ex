defmodule Golf.Game.Player do
  ...
  @type id :: integer
  @type hand_card :: %{card: Card.t(), face_up?: boolean}
  ...
  def give_cards(player, cards) do
    hand = Enum.map(cards, fn card -> %{card: card, face_up?: false} end)
    %Player{player | hand: hand}
  end

  @spec flip_card(t, non_neg_integer) :: t
  def flip_card(player, index) do
    hand = List.update_at(player.hand, index, fn hand_card -> %{hand_card | face_up?: true} end)
    %Player{player | hand: hand}
  end
  ...
  @spec swap_card(Player.t(), integer) :: {Card.t(), Player.t()}
  def swap_card(%{held_card: held_card} = player, index) when is_binary(held_card) do
    %{card: card} = Enum.at(player.hand, index)
    hand = List.replace_at(player.hand, index, %{card: held_card, face_up?: true})
  ...
  defp rank_or_none(%{card: <<rank, _suit>>, face_up?: true}), do: rank
  defp rank_or_none(_hand_card), do: :none
  ...
  def num_cards_face_up(player) do
    Enum.count(player.hand, fn hand_card -> hand_card.face_up? end)
  end
